...
import { X, CheckCircle, Loader2, Phone, User, Car, Zap, Settings, Briefcase, AlertCircle } from 'lucide-react';
...
export const QuoteModal: React.FC<QuoteModalProps> = ({ isOpen, onClose, selectedPackage }) => {
  ...
  useGSAP(() => {
    if (submitted) {
       gsap.fromTo(".modal-success-icon", 
         { scale: 0, rotation: -180 }, 
         { scale: 1, rotation: 0, duration: 0.8, ease: "back.out(1.7)" }
       );
       gsap.fromTo(".modal-success-text",
         { y: 20, opacity: 0 },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNameValid && isPhoneValid && isVehicleValid) {
      setLoading(true);
  ...
  const getPackageIcon = () => {
    switch(selectedPackage) {
      case 'Premium': return <Zap className="text-brand-500" size={24} />;
      case 'Fleet': return <Briefcase className="text-blue-400" size={24} />;
  ...
  const getPackageColor = () => {
    switch(selectedPackage) {
      case 'Premium': return 'text-brand-500 bg-brand-500/10 border-brand-500/30';
      case 'Fleet': return 'text-blue-400 bg-blue-500/10 border-blue-500/30';
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-white/10 flex justify-between items-center bg-[#0f0f0f]">
          <h3 className="text-lg font-bold text-white tracking-wide">{t('offer.book_quote')}</h3>
          <button 
            onClick={handleClose}
        ...
        <div className="p-6">
          {submitted ? (
            <div className="flex flex-col items-center justify-center text-center py-8">
              ...
              <h4 className="modal-success-text text-white font-bold text-2xl mb-2">{t('booking.success')}</h4>
              <p className="modal-success-text text-neutral-400 text-sm max-w-[250px] mb-8">{t('booking.success_sub')}</p>
              
              <div className="modal-success-text w-full bg-emerald-900/10 border border-emerald-500/20 rounded-lg p-4 mb-6">
                 <p className="text-xs text-emerald-400 font-bold uppercase tracking-wider mb-1">Quote Reference</p>
                 <p className="text-lg text-white font-mono">#{quoteId}</p>
              </div>

              <Button fullWidth onClick={handleClose} variant="outline" className="border-white/10 hover:bg-white/5">
                Close
              </Button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className={`p-4 rounded-xl border flex items-center gap-4 ${getPackageColor()}`}>
                 <div className="p-2 bg-black/20 rounded-lg">
                    {getPackageIcon()}
                 </div>
                 <div>
                    <p className="text-[10px] font-mono uppercase tracking-wider opacity-80">Selected Package</p>
                    <p className="text-lg font-bold leading-none mt-0.5">{selectedPackage} Service</p>
                 </div>
              </div>
              ...
              <div className="pt-2">
                 <Button type="submit" fullWidth size="lg" disabled={loading} className="py-4 text-base shadow-[0_4px_20px_rgba(213,243,101,0.25)] hover:shadow-[0_4px_30px_rgba(213,243,101,0.4)]">
                   {loading ? <Loader2 className="animate-spin w-5 h-5 mx-auto" /> : t('offer.book_quote')}
                 </Button>
