...
export const BookingForm: React.FC<{ id?: string }> = ({ id }) => {
  ...
  const [serviceType, setServiceType] = useState<"premium" | "standard">(
    "premium",
  );
    ...
    if (isNameValid && isPhoneValid && isVehicleValid && isDateValid) {
      ...
      try {
        ...
        if (response.success) {
          ...
          setLoading(false);
          alert(
            response.message || "Booking submission failed. Please try again.",
          );
        }
      } catch (error) {
    ...
    >
      ...
      <div className="p-6 pb-4 border-b border-white/10 relative z-10 bg-[#0a0a0a]">
        <div className="flex items-center gap-4 mb-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-white tracking-wide leading-none mb-1">
              {t("booking.title")}
            </h3>
      ...
      <div className="p-6 pt-6 relative z-10 flex-1 flex flex-col bg-[#111]">
        {submitted ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
            ...
            <h4 className="success-text text-white font-bold text-3xl mb-4">
              {t("booking.success")}
            </h4>
            <p className="success-text text-lg text-neutral-300 max-w-[280px] leading-relaxed mb-8">
            ...
            <div className="success-text w-full max-w-xs bg-emerald-900/20 border border-emerald-500/20 rounded-lg p-4 flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-[0.9375rem] text-emerald-400 font-bold uppercase tracking-wider">
                  SMS Confirmation
                </p>
                <p className="text-[0.9375rem] text-white">
                  Sent to {formatNumber(formData.phone)}
                </p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <Zap
                  size={22}
                  className={`mb-2 transition-colors ${serviceType === "premium" ? "text-brand-500" : "text-neutral-500 group-hover:text-neutral-300"}`}
                />
                <p
                  className={`text-[0.9375rem] font-bold uppercase tracking-wider ${serviceType === "premium" ? "text-white" : "text-neutral-400 group-hover:text-neutral-300"}`}
                >
                  {t("booking.premium")}
                </p>
              </div>
              ...
              >
                ...
                <Droplet
                  size={22}
                  className={`mb-2 transition-colors ${serviceType === "standard" ? "text-brand-500" : "text-neutral-500 group-hover:text-neutral-300"}`}
                />
                <p
                  className={`text-[0.9375rem] font-bold uppercase tracking-wider ${serviceType === "standard" ? "text-white" : "text-neutral-400 group-hover:text-neutral-300"}`}
                >
                  {t("booking.standard")}
                </p>
              </div>
            ...
            <div className="space-y-3.5 flex-grow">
              ...
              <div className="relative group">
                <input
                  ...
                  className={`w-full bg-[#1a1a1a] border rounded-xl px-4 py-3 text-white text-[0.9375rem] outline-none transition-all placeholder:text-neutral-600 [color-scheme:dark] [&::-webkit-calendar-picker-indicator]:opacity-60 [&::-webkit-calendar-picker-indicator]:cursor-pointer [&::-webkit-calendar-picker-indicator]:hover:opacity-100 focus:bg-[#222] ${errors.date ? "border-red-500 focus:border-red-500 focus:ring-1 focus:ring-red-500" : "border-white/10 focus:border-brand-500/50 focus:ring-1 focus:ring-brand-500/50"}`}
                />
              </div>
            </div>
            ...
            <div className="mt-2">
              ...
              <p className="text-center text-[0.9375rem] text-neutral-500 mt-3 flex items-center justify-center gap-1.5 font-medium">
                <ShieldCheck size={12} className="text-emerald-500" /> 100% Free
                Cancellation · No Upfront Payment
              </p>
            </div>
          </form>
        )}
