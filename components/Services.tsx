...
export const Services: React.FC = () => {
  ...
  const services = useMemo(() => [
    {
      ...
      specs: { time: formatNumber('15') + ' Min', interval: formatNumber('5,000') + ' KM' }
    },
    {
      id: 'battery',
      icon: <Zap size={24} />,
  ...
  useGSAP(() => {
    // Content entrance
    gsap.fromTo(".service-content-text",
      { opacity: 0, x: 20 },
      { opacity: 1, x: 0, duration: 0.5, stagger: 0.1, ease: "power2.out" }
    );
    
    // Image scale effect
    gsap.fromTo(".service-image-bg",
    ...
    gsap.fromTo(".spec-card",
      { y: 20, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.4, stagger: 0.1, delay: 0.2, ease: "back.out(1.5)" }
    );
  ...
  return (
    <Section className="border-b border-white/5 bg-[#030303] relative overflow-hidden">
      ...
      <div className="relative z-10 mb-16 text-center md:text-start flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
           <div className="inline-flex items-center gap-2 text-brand-500 font-mono text-xs uppercase tracking-widest mb-4">
              <Settings size={14} className="animate-spin-slow" />
              <span>Core Maintenance Modules</span>
           </div>
           <h2 className="text-4xl md:text-6xl font-bold text-white mb-2">{t('services.title')}</h2>
          ...
          {services.map((service) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                   <div ref={progressRef} className="absolute bottom-0 left-0 h-[2px] bg-brand-500 z-10"></div>
                )}

                <div className={`relative z-10 p-3 rounded-lg transition-colors duration-300 ${isActive ? 'bg-brand-500 text-black shadow-lg shadow-brand-500/20' : 'bg-black/40 text-neutral-600 group-hover:text-neutral-400'}`}>
                  {/* Icon Wrapper with Animation - Conditioned hover color to prevent disappearing on active state */}
                  <div className={`transition-all duration-500 ease-out group-hover:scale-110 group-hover:rotate-12 ${isActive ? 'text-black' : 'group-hover:text-brand-500'}`}>
                     {service.icon}
                  </div>
                </div>
                
                <div className="relative z-10 flex-1">
                   <span className={`block font-bold text-lg tracking-wide transition-colors ${isActive ? 'text-white' : 'text-neutral-400 group-hover:text-white'}`}>
                      {service.title}
                   </span>
                </div>
                
                {/* Active Indicator Arrow */}
                <div className={`relative z-10 transition-all duration-300 ${isActive ? 'opacity-100 translate-x-0 text-brand-500' : 'opacity-0 -translate-x-4'}`}>
                   <ArrowRight size={20} />
                </div>
           ...
           <div className="relative z-30 h-full flex flex-col justify-between p-8 md:p-12">
              ...
              <div className="flex gap-2 md:gap-3 justify-end">
                  <div className="spec-card px-2.5 py-1 md:px-4 md:py-2 bg-black/60 backdrop-blur-md border border-white/10 rounded-lg flex items-center gap-2 md:gap-3">
                     ...
                     <div>
                        <p className="text-[8px] md:text-[10px] text-neutral-500 uppercase font-mono leading-none mb-0.5 md:mb-1">Duration</p>
                        <p className="text-[10px] md:text-sm font-bold text-white leading-none">{activeData.specs.time}</p>
                     </div>
                  </div>
                  <div className="spec-card px-2.5 py-1 md:px-4 md:py-2 bg-black/60 backdrop-blur-md border border-white/10 rounded-lg flex items-center gap-2 md:gap-3">
                     <Repeat className="w-3 h-3 md:w-4 md:h-4 text-brand-500" />
                     <div>
                        <p className="text-[8px] md:text-[10px] text-neutral-500 uppercase font-mono leading-none mb-0.5 md:mb-1">Interval</p>
                        <p className="text-[10px] md:text-sm font-bold text-white leading-none">{activeData.specs.interval}</p>
                     </div>
                  </div>
              </div>
