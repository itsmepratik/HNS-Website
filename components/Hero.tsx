...
const HERO_BG =
  "https://images.unsplash.com/photo-1625047509168-a7026f36de04?q=80&w=2000&auto=format&fit=crop";
...
export const Hero: React.FC = () => {
  ...
  // Preload hero background for immediate display
  useEffect(() => {
  ...
  useEffect(() => {
    // Generate a random number between 4 and 14 on mount
    setSpotsLeft(Math.floor(Math.random() * 11) + 4);
  }, []);
  ...
  const handleCallClick = () => {
    window.location.href = "tel:+96871170805"; // Replace with actual number
  };
  ...
  useGSAP(
    () => {
      ...
      gsap.set(
        ".hero-badge, .hero-title-line, .hero-subtitle, .hero-trust-badge, .hero-actions, .hero-stat-card, .hero-form-container",
        { autoAlpha: 1 },
      );
      ...
      tl.from(
        ".hero-title-line",
        { y: 50, autoAlpha: 0, duration: 1, stagger: 0.15 },
        "-=0.6",
      );
  ...
  return (
    <div
      ref={heroRef}
      className="relative pt-24 pb-8 md:pt-32 px-4 sm:px-6 bg-[#050505] overflow-hidden"
    >
      <div className="w-full">
        ...
        <div className="relative rounded-3xl overflow-hidden min-h-[auto] lg:min-h-[85vh] flex items-center shadow-[0_0_80px_-20px_rgba(213,243,101,0.1)] border border-white/10 bg-[#080808] group">
          ...
          {/* Content */}
          <div className="relative z-10 w-full px-4 sm:px-12 lg:px-16 py-12 md:py-20">
            <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-center">
              {/* Left Column: Text */}
              <div className="lg:col-span-7 text-center lg:text-start flex flex-col items-center lg:items-start">
                {/* Premium Status Badge */}
                <div className="hero-badge inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-brand-500/30 bg-brand-500/10 backdrop-blur-md text-brand-400 text-[0.9375rem] font-semibold mb-8 shadow-[0_0_20px_rgba(213,243,101,0.15)]">
                  <Star className="w-3.5 h-3.5 fill-brand-500 text-brand-500" />
                  {t("hero.status")}
                </div>

                {/* Responsive Typography: INCREASED MOBILE FONT SIZE */}
                <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight text-white mb-6 md:mb-8 leading-[0.95]">
                  <span className="hero-title-line block drop-shadow-lg">
                    {t("hero.title_1")}
                  </span>
                  <span className="hero-title-line block text-transparent bg-clip-text bg-gradient-to-r from-brand-300 via-brand-500 to-brand-600 drop-shadow-[0_0_30px_rgba(213,243,101,0.3)]">
                    {t("hero.title_2")}
                  </span>
                </h1>
                ...
                {/* 
                   UPDATED STRUCTURE: 
                   Outer div centers the block on mobile (justify-center).
                   Inner div is a flex column on mobile (flex-col) and row on sm+ (sm:flex-row).
                   'items-start' on inner div ensures that when stacked vertically, they align to the left edge of their container.
                   Since it's flex-col, the width of the inner div shrinks to fit content, so the badges look aligned relative to each other.
                */}
                  ...
                  <div className="flex flex-col sm:flex-row gap-5 sm:gap-8 items-start sm:items-center">
                    ...
                    <div className="hero-trust-badge flex items-center gap-3 transition-opacity hover:opacity-80 cursor-default">
                      ...
                      <div className="flex flex-col items-start leading-none min-w-0 text-left">
                        <span className="text-white text-[0.9375rem] font-bold mb-0.5 tracking-wide">
                          {t("hero.google_rating")}
                        </span>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[0.9375rem] text-neutral-400 font-medium">
                            {t("hero.google_sub")}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="hero-trust-badge flex items-center gap-3 transition-opacity hover:opacity-80 cursor-default">
                      ...
                      <div className="flex flex-col items-start leading-none min-w-0 text-left">
                        ...
                        <span className="text-[0.9375rem] text-neutral-400 font-medium uppercase tracking-wider">
                          {t("hero.award_sub")}
                        </span>
                      </div>
                    </div>
                ...
                <div className="hero-actions flex flex-col gap-6 justify-center lg:justify-start items-center lg:items-start mb-12 md:mb-16 w-full md:w-auto">
                  ...
                  <div className="flex flex-col sm:flex-row gap-6 w-full sm:w-auto">
                    <Button
                      ...
                      className="w-full sm:w-auto min-w-[200px] shadow-[0_0_40px_rgba(213,243,101,0.3)] hover:shadow-[0_0_60px_rgba(213,243,101,0.5)] border border-brand-400/50"
                    >
                      {t("hero.cta")}
                      <ArrowIcon
                        className={`w-5 h-5 transition-transform ${dir === "rtl" ? "mr-2" : "ml-2"}`}
                      />
                    </Button>
                    ...
                    <Button
                      ...
                      className="w-full sm:w-auto min-w-[160px] border-white/20 hover:bg-white/10 text-white transition-colors backdrop-blur-sm"
                    >
                      <Phone
                  ...
                  {/* Modern Spots Remaining Indicator with Dynamic Count */}
                  <div className="flex items-center gap-3 px-4 py-2 bg-[#0f0f0f]/80 border border-white/5 rounded-full backdrop-blur-md shadow-xl">
                    ...
                    <p className="text-[0.9375rem] text-neutral-400 font-medium">
                      <span className="text-white font-bold font-mono text-base tracking-tight">
                        {formatNumber(spotsLeft)} {t("hero.spots")}
                      </span>{" "}
                      {t("hero.spots_rem")}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-lg lg:max-w-none">
                  ...
                  <div className="hero-stat-card bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex flex-row sm:flex-col items-center sm:items-start gap-4 sm:gap-0 text-left hover:bg-white/10 transition-colors group hover:scale-[1.02] duration-300">
                    ...
                    <div>
                      <p className="text-white text-xl md:text-2xl font-bold font-mono tracking-tight">
                        {formatNumber("30%")}
                      </p>
                      <p className="text-[0.9375rem] text-neutral-400 uppercase tracking-wider leading-tight mt-1">
                        {t("hero.stat_1")}
                      </p>
                    </div>
                  </div>
