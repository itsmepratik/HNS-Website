...
const NavButton = ({
  ...
  >
    <span className="relative z-10 font-medium tracking-wide text-[0.9375rem]">
      {label}
    </span>
    ...
    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 scale-90 translate-y-[-10px] group-hover:opacity-100 group-hover:scale-100 group-hover:translate-y-0 transition-all duration-300 ease-out pointer-events-none whitespace-nowrap z-[100]">
      <div className="bg-[#0f0f0f]/95 backdrop-blur-xl border border-white/10 text-[0.9375rem] uppercase tracking-widest text-brand-50 px-3 py-2 rounded shadow-[0_10px_30px_-5px_rgba(0,0,0,0.8)] relative border-b-brand-500/50">
        {tooltip}
        {/* Arrow */}
      ...
      >
        <div className="flex flex-col">
          ...
          <div className="relative flex justify-between items-center py-3 px-6">
            ...
            >
              <span className="text-lg font-display font-bold text-white tracking-widest inline-block group-hover:scale-105 transition-transform uppercase">
            ...
            <div className="flex items-center gap-4 z-20">
              <div className="hidden md:flex items-center gap-4">
                <button
                  ...
                  className="flex items-center gap-1.5 text-[0.9375rem] font-medium text-neutral-400 hover:text-white transition-colors px-2"
                >
                  <Globe size={18} />
                  <span className="uppercase tracking-wider font-mono text-[0.9375rem]">
                    {language}
                  </span>
                </button>
                <Button
              ...
              >
                <div className="relative w-6 h-6 flex items-center justify-center">
                  <div
                    className={`absolute transition-all duration-300 ${isMobileMenuOpen ? "rotate-90 opacity-0 scale-50" : "rotate-0 opacity-100 scale-100"}`}
                  >
                    <Menu size={24} />
                  </div>
                  <div
                    className={`absolute transition-all duration-300 ${isMobileMenuOpen ? "rotate-0 opacity-100 scale-100" : "-rotate-90 opacity-0 scale-50"}`}
                  >
                    <X size={24} />
                  </div>
                </div>
          ...
          <div
            ref={menuRef}
            className="hidden lg:hidden border-t border-white/10 bg-black/95 backdrop-blur-xl overflow-hidden rounded-b-2xl"
          >
            <div className="flex flex-col p-6 gap-4">
              ...
              <div className="flex items-center justify-between mobile-nav-item px-4">
                ...
                >
                  <Globe size={18} />
                  <span className="uppercase font-mono text-[0.9375rem]">
                    {language}
                  </span>
                </button>
              </div>
