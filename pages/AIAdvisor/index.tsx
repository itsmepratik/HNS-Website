...
import { Cpu, Thermometer, MapPin, Gauge, AlertCircle, RefreshCw, Activity, Zap, CheckCircle, GripHorizontal, Wrench, AlertTriangle } from 'lucide-react';
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const prompt = `
        ...
        2. Identify 3 potential common maintenance issues specifically for this vehicle at this mileage and weather condition.
        3. Suggest 3 specific services relevant to these conditions.
        
        CRITICAL FORMATTING INSTRUCTIONS:
        - "technical_specs" must be an array of strings formatted exactly like: "Factory: [Value]" or "Recommendation: [Value]". 
        - Example: ["Factory: 5W-20", "Recommendation: 5W-30 (Due to Heat/Mileage)", "Capacity: 4.5 Quarts"]
        - "maintenance_protocols" must be an array of short, actionable strings.
        ...
        - technical_specs: Array of strings (e.g. ["Factory: 0W-20", "Recommendation: 5W-30"]).
        - maintenance_protocols: Array of strings (e.g. ["Interval: 5,000KM", "Filter: High Efficiency"]).
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            ...
            required: ["title", "viscosity", "oil_type", "grade", "technical_specs", "maintenance_protocols", "common_issues", "recommended_services"],
          },
        },
      });
       ...
       <Section className="relative z-10 flex-grow flex items-center">
         <div className="max-w-[1400px] mx-auto w-full">
           <div className="text-center mb-12">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded border border-brand-500/40 bg-brand-500/5 text-brand-500 text-xs font-mono tracking-[0.2em] uppercase mb-6 shadow-[0_0_20px_rgba(213,243,101,0.15)]">
                 ...
                 <div className="p-6 flex-grow">
                   <form onSubmit={handleAnalyze} className="space-y-6 h-full flex flex-col">
                      <div className="grid grid-cols-2 gap-5">
                        <div className="group">
                          <label className="text-[10px] text-brand-500 font-mono uppercase mb-1.5 block tracking-wider">{t('ai.make')}</label>
                          <input required name="make" value={formData.make} onChange={handleInputChange} className="w-full bg-[#151515] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-brand-500 focus:bg-brand-500/5 outline-none transition-all font-mono" placeholder="TOYOTA" />
                        </div>
                        <div className="group">
                          <label className="text-[10px] text-brand-500 font-mono uppercase mb-1.5 block tracking-wider">{t('ai.model')}</label>
                          <input required name="model" value={formData.model} onChange={handleInputChange} className="w-full bg-[#151515] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-brand-500 focus:bg-brand-500/5 outline-none transition-all font-mono" placeholder="CAMRY" />
                        </div>
                      </div>
                      
                      <div className="grid grid-cols-2 gap-5">
                        <div className="group">
                          <label className="text-[10px] text-brand-500 font-mono uppercase mb-1.5 block tracking-wider">{t('ai.year')}</label>
                          <input required type="number" name="year" value={formData.year} onChange={handleInputChange} className="w-full bg-[#151515] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-brand-500 focus:bg-brand-500/5 outline-none transition-all font-mono" placeholder="2023" />
                        </div>
                        <div className="group">
                          <label className="text-[10px] text-brand-500 font-mono uppercase mb-1.5 block tracking-wider">{t('ai.mileage')}</label>
                          <input required name="mileage" value={formData.mileage} onChange={handleInputChange} className="w-full bg-[#151515] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-brand-500 focus:bg-brand-500/5 outline-none transition-all font-mono" placeholder="45000" />
                        </div>
                      </div>

                      <div className="group">
                        <label className="text-[10px] text-brand-500 font-mono uppercase mb-1.5 block tracking-wider">{t('ai.location')}</label>
                        <div className="relative">
                          <MapPin className="absolute left-3.5 top-3.5 text-neutral-600 w-4 h-4" />
                          <input required name="location" value={formData.location} onChange={handleInputChange} className="w-full bg-[#151515] border border-white/10 rounded-lg pl-10 pr-4 py-3 text-white text-sm focus:border-brand-500 focus:bg-brand-500/5 outline-none transition-all font-mono" placeholder="OMAN, SOHAR" />
                        </div>
                      </div>
                      ...
                      <div className="pt-4 mt-auto">
                        <Button type="submit" fullWidth disabled={loading} className="relative overflow-hidden group bg-brand-500 hover:bg-brand-400 text-black border-none py-4 shadow-[0_0_20px_rgba(213,243,101,0.2)]">
                            {loading ? (
                            <span className="flex items-center gap-2 font-mono uppercase tracking-widest text-sm font-bold">
                   ...
                   {result ? (
                      <div className="result-dashboard h-full p-8 flex flex-col gap-6 relative z-10">
                         ...
                         <div className="result-header flex justify-between items-start border-b border-white/10 pb-6">
                            <div className="space-y-2">
                               <h3 className={`${dynamicTheme.color} font-mono text-xs uppercase tracking-[0.2em] flex items-center gap-2`}>
                            ...
                            <div className="text-right shrink-0">
                               <p className="text-[10px] text-neutral-500 font-mono uppercase tracking-wider mb-1">{t('ai.confidence')}</p>
                               <div className="flex items-center justify-end gap-2">
                                  <div className="w-24 h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                                     <div className={`w-[98%] h-full ${dynamicTheme.accent}`}></div>
                                  </div>
                                  <span className={`${dynamicTheme.color} font-mono text-sm font-bold`}>98%</span>
                               </div>
                            </div>
                         </div>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="result-card-item bg-[#111]/80 backdrop-blur-sm border border-white/10 rounded-lg p-5 group hover:border-white/20 transition-colors">
                               <h4 className="text-xs font-bold text-neutral-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                  <Activity size={14} className="text-blue-500" /> Technical Specs
                               </h4>
                               <ul className="space-y-2">
                                  {result.technical_specs?.map((spec: string, idx: number) => (
                                     <li key={idx} className="result-list-item text-xs text-neutral-300 flex items-start gap-2">
                                        <span className="w-1 h-1 rounded-full bg-blue-500 mt-1.5 shrink-0"></span> {spec}
                                     </li>
                                  ))}
                               </ul>
                            </div>
                            ...
                            <div className="result-card-item bg-[#111]/80 backdrop-blur-sm border border-white/10 rounded-lg p-5 group hover:border-white/20 transition-colors">
                               <h4 className="text-xs font-bold text-neutral-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                  <Zap size={14} className={dynamicTheme.color} /> Maintenance Protocols
                               </h4>
                               <ul className="space-y-2">
                                  {result.maintenance_protocols?.map((proto: string, idx: number) => (
                                     <li key={idx} className="result-list-item text-xs text-neutral-300 flex items-start gap-2">
                                        <span className={`w-1 h-1 rounded-full ${dynamicTheme.accent.replace('bg-', 'bg-')} mt-1.5 shrink-0`}></span> {proto}
                                     </li>
                                  ))}
                               </ul>
                            </div>
                      ...
                      <div className="h-full flex flex-col items-center justify-center text-neutral-600 space-y-6">
                         ...
                         <div className="text-center max-w-xs mx-auto">
                            <p className="font-mono text-sm uppercase tracking-[0.2em] text-neutral-500 mb-2">System Standby</p>
                            <p className="text-xs text-neutral-700">Enter vehicle telemetry data to initialize diagnostic core and receive AI analysis.</p>
                         </div>
                      </div>
                   )}
                </div>
                {error && (
                   <div className="mt-4 p-4 bg-red-950/30 border border-red-500/20 rounded-lg text-red-400 text-xs font-mono flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                      <AlertCircle size={16} />
                      {error}
                   </div>
                )}
